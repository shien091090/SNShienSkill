...
狀態: concept, 第 1 回合
請讀取: <game>/concept.md
任務: 從數值角度評估: (1) 這個循環要成立需要哪些參數, 列名稱與合理區間 (2) 哪個參數最敏感, 為什麼 (3) 目標單局時長你建議多少, 推算依據 (4) 你預見的崩壞點 (5) 你的推算依賴哪些假設, 試玩時要記錄哪些欄位(逐步與逐局)才能校正它們
輸出格式: 依上列五點分段, 參數與埋點欄位用表格(欄位 / 記錄時機 / 用來校正哪個假設)
```
...
- 執行守則中標記 concept 的條目(R1: 在「確定要有的」與「明確不做的」寫死用哪一種輸入; R3: 「埋點」段寫入數值第 5 點的欄位, 加上驗證問題要用的讀數)
