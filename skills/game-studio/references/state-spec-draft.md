...
任務: 依定案結果寫 Demo 玩法規格, 存到 <game>/spec.md。「操作」章節只列 decisions.md 選定的那一種輸入, 每個玩家動作都要有對應。只寫玩法, 不寫美術表現、不寫音效、不寫技術實作。「數值參數表」以定案的數值方向為底給出具體初始值, 並包含畫面寬與畫面高。「物件清單」列出所有會出現在畫面上、玩家需要辨識的東西與它們的狀態。「Demo 範圍邊界」把定案的「明確不做的」抄進來並補上你認為 Demo 不該碰的。
輸出格式: 章節照下列模板, 不增刪:
...
- 執行守則中標記 spec-draft 的條目(R1: 操作章節只有一種輸入; R2: guide.md 存在, 每頁有標題、文字、圖要示意什麼)
