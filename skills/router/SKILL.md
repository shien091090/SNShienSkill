...
> ## ⚠ Local install notes (this machine)
>
> **1. Layout is flattened.** Skills are installed at `~/.claude/skills/<skill-name>/`, NOT at
> the `skills/<group>/<name>/` paths in the tables below. Those grouped paths are labels for
> identification only — do not try to read them. Every skill here is registered with Claude Code
> by name, so load one with the **Skill tool using its bare name** (`unity-physics`, `game-feel`,
> `platformer`), never by file path.
>
> **2. Only a subset is installed.** Present: `unity` (8), `disciplines` (15), `genres` (9),
> `workflows` (4), and this router. **NOT installed:** `godot`, `unreal`, `web-engines`
> (Phaser/PixiJS/three.js), `other-engines` (Bevy/pygame/LÖVE/Roblox). If detection lands on a
> missing engine, say so plainly and offer to install that group — do not silently substitute
> another engine or invent the missing skill's content.
>
> **3. Default engine is Unity, not Godot.** Step 6's fallback says "default to Godot"; on this
> machine ask, or assume **Unity 6.3 LTS**, which is what is installed.
