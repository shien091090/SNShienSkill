...
Requirement: `python -c "import PIL"` — if missing, `python -m pip install --user pillow`.

**Use `python`, never `python3`.** On Windows `python3` is a Microsoft Store *App Execution
Alias* stub (`...\WindowsApps\python3.exe`): it is on PATH, produces no output, and exits 9009 —
so every `python3 ...` command fails *silently* and the loop below looks like it did nothing.
All commands in this file therefore say `python`. On macOS/Linux where `python` may be absent or
Python 2, substitute `python3` there.

**Import path.** Build scripts must reach `scripts/pixelstudio.py`. Use the platform-agnostic
form so the same `build.py` runs on every machine this skill is synced to:

```python
import sys, os; sys.path.insert(0, os.path.expanduser("~/.claude/skills/pixel-art-studio/scripts"))
```

**Opening deliverables.** `open <file>` is macOS-only. Use `start <file>` on Windows (cmd), or
just report the path and let the user click it.
...
2. **Build script** — create `pixel-art/<slug>/build.py` in the working directory.
   ...
   ```
3. **Run** — `cd pixel-art/<slug> && python build.py`
...
1. `python <skill>/scripts/study.py <file> [--save-palette <name>]`
...
```bash
python <skill>/scripts/pixelpipe.py <gen.png> --strip-checker --max-colors 24 --display 6
# lock to a shared game palette for cross-asset consistency:
python <skill>/scripts/pixelpipe.py <gen.png> --palette my_game
# learn the style while cleaning:
python <skill>/scripts/pixelpipe.py <gen.png> --strip-checker --study knight_style
```
...
- Deliverables get opened when done (`start` on Windows, `open` on macOS); report includes the 1x master path.
