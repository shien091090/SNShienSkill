...
```
stage: 頁面內容          // 堆素材 | 分topic | 講稿 | 頁面內容 | 版型定案 | 生簡報 | 完成
title: <簡報名稱>
...
log:
- 2026-09-07 分 topic 定案, 6 份素材, 1 份拆分
```

- `stage`: 目前所在階段; 「完成」= `output/<title>.pptx` 已確認無誤, skill 結束
- topic 表: 一行一個 topic, `#` 兩位數與資料夾前綴一致
- 「講稿」/「頁面內容」欄位值: `-` 未開始 / `doing` / `done` / `stale` (前面階段被回頭改過, 這欄要重看)
...
- 圖片描述**開頭要標來源**: `@photo` (使用者提供的照片或截圖) / `@ai` (較複雜, 需 AI 生圖) / `@svg` (較簡單, 可向量畫)。`--images-todo` 依這個標籤分組, 每組附各自的處理方式。標籤在階段 4 拆圖時就標, 不要等到要補圖才回頭補
- `| a | b |` 表格列 → 對應 role `left` / `right`, 第一列是欄標題 (會加粗); `|---|` 分隔列可有可無
- `> ` 開頭 → speaker notes, 多行會合併; `>` 後面有沒有空格都可以。階段 6 會把 SCRIPT.md 的講稿寫進這裡, 見 SKILL.md 階段 6
...
由 `build_pptx.py <deck> --images-todo` 產生, 不手寫。依圖片描述開頭的 `@photo` / `@ai` / `@svg` 分組, 沒標籤的集中在「(未標籤)」一組提醒回去補。
...
- `decor` role: 版型固定的裝飾, **不吃頁面內容**, 值全部寫在 STYLE.md。用在章節頁的色塊短線、圖與圖之間的箭頭、疊在圖上的固定標籤這類東西。額外欄位: `shape` (`rect` 預設 / `arrow` / `arrow-down` / `oval` / `text`)、`color` 圖形填色 (預設 theme.accent)、`text` 圖形內或文字框的文字、`text_color` 字色 (預設 theme.bg)、`size` 字級、`bold`。`shape: text` 是純文字框沒有底色, 適合疊在圖上; 其他 shape 是實心圖形。同一版型可以放多個 decor
  生成圖不含文字與箭頭是刻意的 (生圖模型畫中文會壞, 箭頭獨立出來才改得動), 這些都用 decor 補回來, 所以重 build 不會掉
