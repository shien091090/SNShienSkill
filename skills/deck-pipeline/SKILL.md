...
- **SCRIPT.md 只放演講稿本身, 不放任何給使用者看的東西**。判準是「講者站在台上會不會用到這行」:
  - 可以放: 章節標題與配時、要講的重點條列、`[這邊放 xxx.jpg]`、`(這段帶過不逐條念)` 這類臨場提示、`[待確認]` 標記
  - **不可以放**: 為什麼這樣寫、砍了什麼與理由、沿用了哪份舊檔、素材待補狀態、給自己的寫作指示 (例如「這段不要舉例」「聽眾含非開發所以技術細節壓掉」)、配時怎麼勻的
  - 這些全部寫進 **STATUS.md 的 log**。不是刪掉 — 決策理由有價值, 只是不屬於演講稿。之後要回頭調整時去 log 裡找, 不用重推一遍
